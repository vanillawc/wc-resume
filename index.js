...
class WCResume extends HTMLElement {
  ...
  async connectedCallback() {
    ...
    template.innerHTML = this.innerHTML === "" ? WCResume.default() : this.innerHTML;
  ...
  async setTheme() {
    ...
    if (this.__contact) {
      this.__contact.template = await this.fetchTheme(path, "contact.html");
    }
    if (this.__about) {
      this.__about.template = await this.fetchTheme(path, "about.html");
    }
    if (this.__profiles) {
      this.__profiles.template = await this.fetchTheme(path, "profiles.html");
    }
    if (this.__skills) {
      this.__skills.template = await this.fetchTheme(path, "skills.html");
    }
    if (this.__work) {
      this.__work.template = await this.fetchTheme(path, "work.html");
    }
    if (this.__projects) {
      this.__projects.template = await this.fetchTheme(path, "projects.html");
    }
    if (this.__education) {
      this.__education.template = await this.fetchTheme(path, "education.html");
    }
    if (this.__publications) {
      this.__publications.template = await this.fetchTheme(path, "publications.html");
    }
    if (this.__awards) {
      this.__awards.template = await this.fetchTheme(path, "awards.html");
    }
    if (this.__volunteer) {
      this.__volunteer.template = await this.fetchTheme(path, "volunteer.html");
    }
    if (this.__languages) {
      this.__languages.template = await this.fetchTheme(path, "languages.html");
    }
    if (this.__interests) {
      this.__interests.template = await this.fetchTheme(path, "interests.html");
    }
    if (this.__references) {
      this.__references.template = await this.fetchTheme(path, "references.html");
    }
  }
  ...
  render() {
    if (this.__contact) {
      this.__contact.data = this.__data.basics;
    }
    if (this.__about) {
      this.__about.data = this.__data.basics;
    }
    if (this.__profiles) {
      this.__profiles.data = this.__data.basics;
    }
    if (this.__skills) {
      this.__skills.data = this.__data.skills;
    }
    if (this.__work) {
      this.__work.data = this.__data.work;
    }
    if (this.__projects) {
      this.__projects.data = this.__data.projects;
    }
    if (this.__education) {
      this.__education.data = this.__data.education;
    }
    if (this.__publications) {
      this.__publications.data = this.__data.publications;
    }
    if (this.__awards) {
      this.__awards.data = this.__data.awards;
    }
    if (this.__volunteer) {
      this.__volunteer.data = this.__data.volunteer;
    }
    if (this.__languages) {
      this.__languages.data = this.__data.languages;
    }
    if (this.__interests) {
      this.__interests.data = this.__data.interests;
    }
    if (this.__references) {
      this.__references.data = this.__data.references;
    }
  }
  static default() {
    return `
